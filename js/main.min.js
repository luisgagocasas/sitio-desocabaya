...
0>timeLeft&&(timeLeft=0);for(key in time){var y=time[key],x=digits[key]=add_leading_zero(Math.floor(timeLeft/y));timeLeft-=x*y}settings.callback(digits.days,digits.hours,digits.minutes,add_leading_zero(timeLeft)),setTimeout(countdown,1e3)}()};var add_leading_zero=function(n){return n.toString().length<2?"0"+n:n};plugin.init()},$.fn.scountdown=function(options){return this.each(function(){new $.scountdown(this,options)})}}(jQuery),$(document).ready(function(){$("#frmcheck").submit(function(){var formulario=$("#frmcheck").serializeArray();return $.ajax({type:"POST",dataType:"json",url:"mailchimp/mail.php",data:formulario}).done(function(respuesta){$("#formulario").html(respuesta.mensaje).fadeIn()}),!1});var $countdown=$(".countdown");$countdown.scountdown({timestamp:parseInt(14275692e5,10),callback:function(d,h,m,s){$(".dias",$countdown).text(d),$(".horas",$countdown).text(h),$(".minutos",$countdown).text(m),$(".segundos",$countdown).text(s)}})}),$("#suscribirse").click(function(){return $("#frmcheck").css("display","block"),!1}),$("#cerrar").click(function(){return $("#frmcheck").css("display","none"),!1});